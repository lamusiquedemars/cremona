                    ...
                    <div class="mt-4 grid gap-4 lg:grid-cols-2">
                        @foreach ($group['changes'] as $change)
                            <section class="rounded-lg border border-amber-200/80 bg-white/80 p-3 dark:border-amber-900/60 dark:bg-gray-900/60"><h5 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $change['label'] }}</h5>
                                <div class="mt-3 grid gap-3"><div><div class="text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Dans Google, pas dans la préparation Cremona</div><div class="mt-1" style="display:flex; flex-wrap:wrap; gap:.4rem;">@forelse($change['added'] as $word)<span class="rounded-full bg-emerald-100 px-2 py-1 text-xs text-emerald-900 dark:bg-emerald-950 dark:text-emerald-100">{{ $word }}</span>@empty<span class="text-sm text-gray-500">—</span>@endforelse</div></div>
                                <div><div class="text-xs font-medium uppercase tracking-wide text-rose-700 dark:text-rose-300">Dans la préparation Cremona, pas dans Google</div><div class="mt-1" style="display:flex; flex-wrap:wrap; gap:.4rem;">@forelse($change['missing'] as $word)<span class="rounded-full bg-rose-100 px-2 py-1 text-xs text-rose-900 dark:bg-rose-950 dark:text-rose-100">{{ $word }}</span>@empty<span class="text-sm text-gray-500">—</span>@endforelse</div></div></div>
                            </section>
                        @endforeach
                    </div>
                </article>
